...
const pages = {
    overview: {
        ...
        fields: [
            ...
            { name: '🛡️ Moderation & AutoMod', value: 'Warning log indexes, time-out silences, kick/ban actions, automated chat content spam blockers.' },
            { name: '🛠️ Onboarding & Utility', value: 'Onboarding cards, components reaction role boards, support portal helpdesks, weather alerts.' },
    ...
    help_grinding: {
        ...
        fields: [
            ...
            { name: '`/use [item]`', value: 'Consumes items for perks (Pizza for 🏆150 XP, Energy Drink for <:coin:1512926963239489606>300 coins, Lootboxes for prize wheels).' },
...
function buildMenu(disabled = false) {
    const menu = new StringSelectMenuBuilder()
        ...
        .addOptions(
            new StringSelectMenuOptionBuilder().setLabel('Galaxy System Overview').setDescription('Bot introduction, core systems specs & categories.').setValue('overview').setEmoji('🌌'),
            new StringSelectMenuOptionBuilder().setLabel('Core & AI Systems').setDescription('/friday ask, rewrite, summarize, customcmd add/remove.').setValue('help_core').setEmoji('⚡'),
            new StringSelectMenuOptionBuilder().setLabel('Economy & Banking').setDescription('/balance, deposit, withdraw, wages, begging, daily.').setValue('help_economy').setEmoji('💰'),
            new StringSelectMenuOptionBuilder().setLabel('Job Ecosystem').setDescription('/job list, apply, quit, profile. Tiered careers that scale /work pay.').setValue('help_jobs').setEmoji('💼'),
            new StringSelectMenuOptionBuilder().setLabel('Casino & Heists').setDescription('/blackjack, slots, roulette, cockfights, heists.').setValue('help_casino').setEmoji('🎲'),
            new StringSelectMenuOptionBuilder().setLabel('Scavenging & Shop').setDescription('/hunt, fish, dig, inventory, shop, players bazaar.').setValue('help_grinding').setEmoji('🎣'),
            new StringSelectMenuOptionBuilder().setLabel('Stocks & Trading').setDescription('/stock list, quote, buy, sell, portfolio open/close/view.').setValue('help_stocks').setEmoji('📈'),
            new StringSelectMenuOptionBuilder().setLabel('Pets & Leveling').setDescription('/pet adopt, feed, train, visual ranks, leaderboards.').setValue('help_pets_leveling').setEmoji('✨'),
            new StringSelectMenuOptionBuilder().setLabel('Moderation & AutoMod').setDescription('/automod, warns, slowmodes, purges, locks, kicks, bans.').setValue('help_moderation').setEmoji('🛡️'),
            new StringSelectMenuOptionBuilder().setLabel('Onboarding & Utility').setDescription('Welcome visual cards, auto-roles, tickets portal, widgets.').setValue('help_utility').setEmoji('🛠️'),
            new StringSelectMenuOptionBuilder().setLabel('Giveaways & Event Logs').setDescription('/giveaway, events, voice locks, audit logs list, streamers alerts.').setValue('help_giveaways').setEmoji('🎉')
        );
