        ...
        try {
            ...
            if (subcommand === 'status') {
                ...
                const container = new ContainerBuilder()
                    ...
                    .addSectionComponents(
                        new SectionBuilder()
                            .addTextDisplayComponents(
                                new TextDisplayBuilder().setContent(`## ⚙️ System Diagnostics & Metrics\n**Core Engine Name:** Friday Core`)
                            )
            ...
            if (subcommand === 'ask') {
                ...
                if (!apiKey || apiKey === 'your_gemini_api_key_here' || apiKey.trim() === '') {
                    return interaction.reply({
                        content: '⚠️ **Gemini API Key is not configured.** Please add a valid `GEMINI_API_KEY` to your `.env` file.',
                        ephemeral: true
                    });
                }
            ...
            if (subcommand === 'summarize') {
                ...
                try {
                    ...
                    if (messages.size === 0) {
                        return interaction.editReply({ content: '⚠️ No messages found in the active channel history to summarize.' });
                    }
                    ...
                    if (chatLogs.trim() === '') {
                        return interaction.editReply({ content: '⚠️ Could not find any readable user text messages in the last 50 entries to summarize.' });
                    }
