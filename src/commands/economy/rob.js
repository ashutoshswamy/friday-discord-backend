  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (robberProfile.coins < 100) {
        return interaction.editReply({ content: `You must possess at least ${EMOJIS.coin} **100** coins to plan a heist!`, ephemeral: true });
      }

      if (victimProfile.coins < 50) {
        return interaction.editReply({
          content: `<@${targetUser.id}> is practically broke! They only have ${EMOJIS.coin} **${victimProfile.coins.toLocaleString()}** coins. Not worth the risk.`,
          ephemeral: true
        });
      }
          ...
          const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
              new TextDisplayBuilder().setContent(
                ...
                `**Compensation Paid:** ${EMOJIS.coin} **-${fineAmount.toLocaleString()}** coins`
              )
            );
