    ...
    return new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(
                ...
                `*Click Join below — requires <:coin:1512926963239489606> 100 coins in your wallet in case you get caught.*`
            )
        )
        ...
        try {
            ...
            if (hostProfile.coins < 100) {
                return interaction.editReply({
                    content: '❌ You must possess at least <:coin:1512926963239489606> **100** coins in your active wallet to fund and organize a heist crew!',
                    ephemeral: true
                });
            }
            ...
            if (victimBankBalance < 200) {
                return interaction.editReply({
                    content: `❌ <@${targetUser.id}>'s bank vault is practically empty! They only have <:coin:1512926963239489606> **${victimBankBalance.toLocaleString()}** coins in the bank. It isn't worth the massive risk.`,
                    ephemeral: true
                });
            }
                ...
                if (i.customId === 'heist_join') {
                    ...
                    try {
                        ...
                        if (clickerProfile.coins < 100) {
                            return i.reply({
                                content: `❌ You need at least <:coin:1512926963239489606> **100** coins in your active wallet to join this high-risk heist! (Current: <:coin:1512926963239489606> ${clickerProfile.coins.toLocaleString()})`,
                                ephemeral: true
                            });
                        }
                ...
                if (isSuccess) {
                    ...
                    const successContainer = new ContainerBuilder()
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `## 🔥 VAULT BREACHED!\n🏆 **Heist Success!**\n\n` +
                                `The crew infiltrated <@${targetUser.id}>'s bank vault and escaped with **<:coin:1512926963239489606> ${stolenAmount.toLocaleString()}** coins!`
                            )
                        )
                        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `**Loot Split (${crewSize} crew members):**\nEach crew member receives <:coin:1512926963239489606> **${splitAmount.toLocaleString()}** coins!\n\n` +
                                `**Accomplices:** ${crewPings}\n\n` +
                                `**Target Loss:** 🏦 **-${stolenAmount.toLocaleString()}** bank coins\n` +
                                `**Split Per Thief:** <:coin:1512926963239489606> **+${splitAmount.toLocaleString()}** coins`
                            )
                        );
                    ...
                    for (const member of crew) {
                        ...
                        fineList.push(`• <@${member.id}>: fined <:coin:1512926963239489606> **${fine.toLocaleString()}** coins`);
                    }
                    ...
                    const failContainer = new ContainerBuilder()
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `**Crew Fines:**\n${fineList.join('\n')}\n\n` +
                                `**Total Compensated to Victim:** <:coin:1512926963239489606> **+${totalCompensation.toLocaleString()}** coins`
                            )
                        );
