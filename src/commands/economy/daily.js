...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const shopBtn = new ButtonBuilder().setCustomId('daily_shop_reminder').setLabel('🛍️ Visit Shop').setStyle(ButtonStyle.Secondary);
            ...
            const container = new ContainerBuilder()
                ...
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(
                        `**🎁 Reward:** <:coin:1512926963239489606> **+${result.reward.toLocaleString()}** coins\n` +
                        `**💰 Wallet Balance:** <:coin:1512926963239489606> **${result.newBalance.toLocaleString()}** coins\n` +
                        `**⏭️ Next Claim:** <t:${Math.floor((Date.now() + 86400000) / 1000)}:R>`
                    )
                )
                .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
                .addTextDisplayComponents(
